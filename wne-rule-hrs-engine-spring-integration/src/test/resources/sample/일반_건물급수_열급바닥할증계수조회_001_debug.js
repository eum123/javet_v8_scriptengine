function(열급바닥면적비율 ){var start = new Date().getTime();
var __hrs_logger_content = [];
var __hrs_logger_input = [];
var __hrs_logger_output = {};
var __hrs_logger_object = {executionTime: 0,ruleId:'일반_건물급수_열급바닥할증계수조회_001',ruleName:'일반_건물급수_열급바닥할증계수조회',ruleType:'SPREAD',content:__hrs_logger_content,input:__hrs_logger_input,output:__hrs_logger_output,};
...
if( __hrs_logger_object.content ){__hrs_logger_object.content.push('['+TODAY_FORMAT(new Date().getTime())+'][INPUT] '+'열급바닥면적비율:'+JSON.stringify(열급바닥면적비율)); if(SERVER_DEBUG){ SERVER_DEBUG(__hrs_logger_object.content[__hrs_logger_object.content.length - 1], __hrs_logger_object.ruleId) }; }
if( __hrs_logger_object.input ){__hrs_logger_object.input.push( { 'name':'열급바닥면적비율' , 'value':JSON.stringify(열급바닥면적비율) } );}
...
var __RULE_INQUIRY_BASE_DATE__ = __TARGET_DATE__;
    ...
    if(((열급바닥면적비율 <= 10))){
    _GET_GLOBAL('__hrs_logic_logger').push({ruleId:'일반_건물급수_열급바닥할증계수조회_001' , row:0});
     __hpasRuleResult = {
'열급바닥할증계수' : 1.1
}
    ...
    _GET_GLOBAL('__hrs_logic_logger').push({ruleId:'일반_건물급수_열급바닥할증계수조회_001' , row:1});
     __hpasRuleResult = {
'열급바닥할증계수' : 1.2
}
    ...
    _GET_GLOBAL('__hrs_logic_logger').push({ruleId:'일반_건물급수_열급바닥할증계수조회_001' , row:2});
     __hpasRuleResult = {
'열급바닥할증계수' : 1.3
}
