function(계속계약할인율_PCT,고액계약할인율_PCT,단체할인율_PCT,부부가족할인율_PCT,세대수할인율_PCT,부보대수할인율_PCT,패키지할인율_PCT,예정사업비율,CM할인율_PCT ){var start = new Date().getTime();
var __hrs_logger_content = [];
var __hrs_logger_input = [];
var __hrs_logger_output = {};
var __hrs_logger_object = {executionTime: 0,ruleId:'일반_공통_사업비적용할인율계산_001',ruleName:'일반_공통_사업비적용할인율계산',ruleType:'LOGIC_TABLE',content:__hrs_logger_content,input:__hrs_logger_input,output:__hrs_logger_output,};
...
if( __hrs_logger_object.content ){__hrs_logger_object.content.push('['+TODAY_FORMAT(new Date().getTime())+'][INPUT] '+'CM할인율_PCT:'+JSON.stringify(CM할인율_PCT)); if(SERVER_DEBUG){ SERVER_DEBUG(__hrs_logger_object.content[__hrs_logger_object.content.length - 1], __hrs_logger_object.ruleId) }; }
if( __hrs_logger_object.input ){__hrs_logger_object.input.push( { 'name':'CM할인율_PCT' , 'value':JSON.stringify(CM할인율_PCT) } );}
